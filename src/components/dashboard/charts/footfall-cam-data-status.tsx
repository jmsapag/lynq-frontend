...
export const FootfallCamDataStatus: React.FC<FootfallCamDataStatusProps> = ({
  ...
  const hasReturningCustomersData = data.returningCustomers.some(
    (val) => val > 0,
  );
  ...
  const hasAnyFootfallCamData =
    hasReturningCustomersData ||
    hasAvgVisitDurationData ||
      ...
      <div className="flex items-start space-x-3">
        ...
        <div>
          ...
          <p className="text-sm text-blue-700">
            Las métricas avanzadas (Clientes que Retornan, Tiempo Promedio en
            Local, y Afluencia) requieren datos de analíticas FootfallCam que no
            están disponibles para este período.
          </p>
          <p className="text-xs text-blue-600 mt-2">
            Estas métricas se mostrarán automáticamente cuando los datos estén
            disponibles desde el sensor.
          </p>
        </div>
      </div>
    </div>
