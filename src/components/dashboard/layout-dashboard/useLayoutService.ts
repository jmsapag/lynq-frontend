...
export const useLayoutService = () => {
  ...
  const saveLayout = useCallback(
    async (placements: Record<string, any>) => {
  ...
  const loadLayout = useCallback(async (): Promise<Record<
    string,
    any
  > | null> => {
  ...
  const createLayout = useCallback(
    async (layout: Omit<DashboardLayout, "id">) => {
      try {
 ...
 */
export const configureLayoutService = (
