...
export const LayoutDashboard: React.FC<LayoutDashboardProps> = ({
  ...
  React.useEffect(() => {
    if (
      ...
      Object.keys(widgetPlacements).length === 0
    ) {
      setWidgetPlacements({
        "metric-1": "total-in",
        "metric-2": "total-out",
      ...
      return (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {widgetPlacements["metric-1"] &&
              renderWidget(widgetPlacements["metric-1"])}
            {widgetPlacements["metric-2"] &&
              renderWidget(widgetPlacements["metric-2"])}
            {widgetPlacements["metric-3"] &&
              renderWidget(widgetPlacements["metric-3"])}
          </div>

          {/* Charts Column */}
          <div className="grid grid-cols-1 gap-6">
            {widgetPlacements["chart-1"] &&
              renderWidget(widgetPlacements["chart-1"])}
            {widgetPlacements["chart-2"] &&
              renderWidget(widgetPlacements["chart-2"])}
            {widgetPlacements["chart-3"] &&
              renderWidget(widgetPlacements["chart-3"])}
          </div>
        </div>
      );
    }
