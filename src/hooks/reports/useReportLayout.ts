...
export const useReportLayout = (): UseReportLayoutReturn => {
  ...
  const [currentLayout, setCurrentLayoutState] = useState<ReportLayoutConfig | null>(() => {
  ...
  const [widgetPlacements, setWidgetPlacements] = useState<ReportWidgetPlacements>(() => {
    const layout = service.getCurrentLayout();
    return layout?.widgetPlacements || {};
  });
  const [draggedWidget, setDraggedWidget] = useState<ReportWidgetType | null>(null);
  ...
  useEffect(() => {
    const loadInitialConfiguration = async () => {
      if (currentLayout) {
        try {
          const savedPlacements = await service.loadLayoutPlacements(currentLayout.id);
          if (savedPlacements) {
  ...
  const setCurrentLayout = useCallback(async (layoutId: string) => {
    ...
    if (layout) {
      ...
      try {
        ...
        console.warn('Failed to load widget placements, using defaults:', error);
        // Use default placements from layout
        setWidgetPlacements(layout.widgetPlacements);
      }
    }
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const widgetCategory = active.data?.current?.category as "metric" | "chart";
    ...
    const isCompatible = targetZoneConfig.type === "any" || targetZoneConfig.type === widgetCategory;
  ...
  const createCustomLayout = useCallback((name: string, description: string) => {
